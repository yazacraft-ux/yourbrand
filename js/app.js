...
async function submitNewMsg() {
  ...
  var newConvId = currentUser.id + '-' + Date.now(); // nouvelle conversation_id unique
  try {
...
async function openConvAdmin(id) {
  ...
  if (!main) return;
  main.innerHTML =
    '<div class="msg-header"><div class="msg-avatar" style="background:linear-gradient(135deg,rgba(124,58,237,0.4),rgba(236,72,153,0.3))">' + conv.initials + '</div><div><p class="msg-name">' + conv.membre + '</p></div></div>' +
