...
function renderDashboardStats() {
  var paidOrders = allOrdersAdmin.filter(function (o) { return o.status === 'paid'; });
...
async function deleteConvMembre(id) {
  ...
  try {
    ...
    if (activeConvId === id) {
      ...
      if (main) main.innerHTML = '<div class="msg-header"><div class="msg-avatar">S</div><div><p class="msg-name">Support YourBrand</p><p class="msg-pack">Sélectionne une conversation</p></div></div>';
...
function renderAdminThreads() {
  ...
  g.innerHTML = adminConversations.map(function (c) {
    ...
    return '<div class="msg-thread' + (activeConvId === c.id ? ' active' : '') + '" onclick="openConvAdmin(\'' + c.id + '\')">' +
      '<div class="msg-thread-name"><span style="display:flex;align-items:center;gap:5px">' + (hasUnread ? '<span class="unread-dot"></span>' : '') + c.membre + '</span><span style="display:flex;align-items:center;gap:6px"><span class="msg-thread-time">' + last.time + '</span><button class="msg-thread-del" title="Supprimer la conversation" onclick="event.stopPropagation();deleteConvAdmin(\'' + c.id + '\')">🗑️</button></span></div>' +
      '<div class="msg-thread-preview">' + last.text + '</div>' +
    '</div>';
  }).join('');
}
...
async function saveAllSettingsToSupabase() {
  var payload = { EP: EP, OB: OB, S: S, trustItems: trustItems, legalData: legalData, MAINT: MAINT, REDIR: REDIR };
  try {
...
async function loadAllSettingsFromSupabase() {
  try {
    ...
    if (data && Object.keys(data).length > 0) {
      ...
      applyLoadedSettingsToDOM();
      cacheSettingsLocally({ EP: EP, OB: OB, S: S, trustItems: trustItems, legalData: legalData, MAINT: MAINT, REDIR: REDIR });
    }
