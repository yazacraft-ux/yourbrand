...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  applyRouteFromHash(true);

  // La page (ou bonne vue : landing / membre / admin) est maintenant déterminée
  // et les bons textes sont en place : on peut révéler le site.
...
/* Détermine quelle vue afficher à partir de l'URL actuelle (#panier, #membre…),
   ...
   - isInitial=true : appelé une seule fois au chargement de la page (refresh/arrivée directe)
   - isInitial=false : appelé à chaque navigation in-app (clic logo, retour navigateur…) */
function applyRouteFromHash(isInitial) {
  ...
  // URL racine : seulement au chargement initial (refresh/arrivée directe) on renvoie
  // un utilisateur connecté vers son espace. Ensuite, cliquer sur le logo doit
  // pouvoir ramener à l'accueil sans être systématiquement renvoyé vers son espace.
  if (isInitial && loggedIn) { showView(currentProfile.role === 'admin' ? 'admin' : 'membre'); return; }
  showView('landing');
}
window.addEventListener('hashchange', function () { applyRouteFromHash(false); });
...
function openConvMembre(id) {
  ...
  if (!main) return;
  main.innerHTML =
    '<div class="msg-header"><button class="msg-back-btn" onclick="msgMobileBack(\'membre-msg-main\')">← Retour</button><div style="display:flex;align-items:center;gap:10px"><div class="msg-avatar">S</div><div><p class="msg-name">Support YourBrand</p><p class="msg-pack">On répond en moins de 24h</p></div></div></div>' +
    '<div class="msg-body" id="conv-body-membre">' + renderMessages(conv) + '</div>' +
    '<div class="msg-footer"><textarea class="msg-input" id="msg-input-membre" placeholder="Écris ton message..."></textarea><button class="msg-send-btn" onclick="sendMsgMembre(\'' + id + '\')">Envoyer →</button></div>';
...
function renderAdminThreads() {
  ...
  g.innerHTML = adminConversations.map(function (c) {
    ...
    return '<div class="msg-thread' + (activeConvId === c.id ? ' active' : '') + '" onclick="openConvAdmin(\'' + c.id + '\')">' +
      '<div class="msg-thread-name"><span style="display:flex;align-items:center;gap:5px">' + (hasUnread ? '<span class="unread-dot"></span>' : '') + c.membre + '</span><span class="msg-thread-time">' + last.time + '</span></div>' +
      '<div class="msg-thread-preview">' + last.text + '</div>' +
    '</div>';
  }).join('');
}
...
async function openConvAdmin(id) {
  ...
  if (!main) return;
  main.innerHTML =
    '<div class="msg-header"><button class="msg-back-btn" onclick="msgMobileBack(\'admin-msg-main\')">← Retour</button><div style="display:flex;align-items:center;gap:10px"><div class="msg-avatar" style="background:linear-gradient(135deg,rgba(124,58,237,0.4),rgba(236,72,153,0.3))">' + conv.initials + '</div><div><p class="msg-name">' + conv.membre + '</p></div></div></div>' +
    '<div class="msg-body" id="admin-conv-body">' + renderMessages(conv) + '</div>' +
    '<div class="msg-footer"><textarea class="msg-input" id="admin-msg-input" placeholder="Ta réponse..."></textarea><button class="msg-send-btn" onclick="sendMsgAdmin(\'' + id + '\')">Répondre →</button></div>';
...
async function loadAllSettingsFromSupabase() {
  try {
    ...
    if (data && Object.keys(data).length > 0) {
      ...
      applyLoadedSettingsToDOM();
      cacheSettingsLocally({ EP: EP, OB: OB, S: S, trustItems: trustItems, legalData: legalData, MAINT: MAINT });
    }
